...
u0 = @(t) [0;0;0];
endt = 100;
[ts_,trajsOrbit] = nonlinearChaserDynamics.simulateMotion(state0,R,u0,endt,0.01); %get the true Trajectories
[ts,trajsTrue] = nonlinearChaserDynamics.simulateMotion(state0,R,u,endt,0.01); %get the true Trajectories
...
for i = 2:length(ts)
    T = ts(i) - ts(i-1);
    [est_state, est_cov] = ChaserEKF.estimate(estimatedTraj(:,i-1), estimatedTrajCov(:,:,i-1), u( ts(i-1) ), T, R, sense_data(:,i), 0.0001*eye(6), 1*eye(3));
